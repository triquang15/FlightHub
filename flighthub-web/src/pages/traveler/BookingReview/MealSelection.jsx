...
const MealSelection = ({ selectedMeals = [], onSelectMeal, totalPassengers = 1 }) => {
  ...
  const passengerLimit = Math.max(Number(totalPassengers) || 1, 1);
    ...
    return normalizeList(flightMeals)
      .map((flightMeal) => {
        ...
        return {
          ...
          currency: String(flightMeal?.currency || meal?.currency || "USD").toUpperCase(),
  ...
  const totalSelectedQuantity = selectedItems.reduce(
    (sum, meal) => sum + (Number(meal.quantity) || 1),
    0,
  );
  ...
  const selectedQuantity = (mealId) =>
    Number(selectedItems.find((meal) => meal.id === mealId)?.quantity) || 0;

  const updateMealQuantity = (meal, nextQuantity) => {
    if (!meal.available) return;
    const currentQuantity = selectedQuantity(meal.id);
    const boundedQuantity = Math.max(0, Math.min(nextQuantity, passengerLimit));
    const nextSelectedTotal = totalSelectedQuantity - currentQuantity + boundedQuantity;
    if (nextSelectedTotal > passengerLimit) return;
    ...
    const existingIndex = selectedItems.findIndex((item) => item.id === meal.id);
    if (existingIndex >= 0) {
      onSelectMeal(
        selectedItems.map((item) =>
          item.id === meal.id ? { ...item, quantity: boundedQuantity } : item,
        ),
      );
      return;
    }

    onSelectMeal([...selectedItems, { ...meal, quantity: boundedQuantity }]);
  };

  const handleMealToggle = (meal) => {
    if (!meal.available) return;
    const quantity = selectedQuantity(meal.id);
    if (quantity > 0) {
      updateMealQuantity(meal, 0);
      return;
    }
    updateMealQuantity(meal, 1);
  };
    ...
    >
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-left sm:text-right dark:border-white/10 dark:bg-slate-950/40">
          ...
          <p className="text-sm font-semibold text-slate-950 dark:text-white">
            {totalSelectedQuantity}/{passengerLimit} meals | {formatCurrency(totalMealCost)}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <AnimatePresence mode="popLayout">
          {displayedMeals.map((meal) => {
            ...
            const isLimitReached = totalSelectedQuantity >= passengerLimit && quantity === 0;
            ...
            return (
              <motion.button
                ...
                disabled={!meal.available || isLimitReached}
              ...
              >
                ...
                <div className="p-4">
                  <div className="mb-2 flex items-start justify-between gap-3">
                    ...
                    >
                      {quantity > 0 ? <span className="text-xs font-bold">{quantity}</span> : <Check className="h-4 w-4" />}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between border-t border-slate-200 pt-3 dark:border-white/10">
                    <span className="text-sm font-bold text-slate-950 dark:text-white">
                      {meal.complimentary ? "Complimentary" : formatCurrency(meal.price, meal.currency)}
                    </span>
                    {isLimitReached ? (
      ...
      {selectedItems.length > 0 ? (
        ...
        >
          <div className="mb-3 flex items-center justify-between gap-3">
            <p className="text-sm font-semibold text-slate-950 dark:text-white">
              Selected meals ({totalSelectedQuantity})
            </p>
            <p className="text-lg font-bold text-emerald-700 dark:text-emerald-300">
              {formatCurrency(totalMealCost)}
            </p>
          </div>
          <div className="mb-3 flex items-center gap-2 rounded-md border border-emerald-200/80 bg-white/70 px-3 py-2 text-xs text-emerald-800 dark:border-emerald-400/20 dark:bg-slate-950/30 dark:text-emerald-100">
            <Users className="h-3.5 w-3.5" />
            Choose up to {passengerLimit} meal{passengerLimit > 1 ? "s" : ""} for this booking.
          </div>
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-3">
                  <span className="font-semibold text-slate-950 dark:text-white">
                    {meal.complimentary
                      ? "Free"
                      : formatCurrency(Number(meal.price || 0) * (Number(meal.quantity) || 1), meal.currency)}
                  </span>
                  <div className="flex items-center rounded-md border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-slate-900">
                    <button
                      type="button"
                      onClick={() => updateMealQuantity(meal, (Number(meal.quantity) || 1) - 1)}
                      className="p-1.5 text-slate-600 hover:text-slate-950 dark:text-slate-300 dark:hover:text-white"
                      aria-label={`Decrease ${meal.name}`}
                    >
                      <Minus className="h-3.5 w-3.5" />
                    </button>
                    <span className="min-w-8 text-center text-xs font-semibold text-slate-950 dark:text-white">
                      {Number(meal.quantity) || 1}
                    </span>
                    <button
                      type="button"
                      onClick={() => updateMealQuantity(meal, (Number(meal.quantity) || 1) + 1)}
                      disabled={totalSelectedQuantity >= passengerLimit}
                      className="p-1.5 text-slate-600 hover:text-slate-950 disabled:cursor-not-allowed disabled:opacity-40 dark:text-slate-300 dark:hover:text-white"
                  ...
                  <button
                    type="button"
                    onClick={() => updateMealQuantity(meal, 0)}
